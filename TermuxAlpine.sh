...
finalwork() {
	...
	[ ! -e ${DESTINATION}/final.sh ] && curl --silent -LO https://raw.githubusercontent.com/Cesar-Hack-Gray/apktool/master/tmp/final.sh
	if [ "${MOTD}" = "ON" ]; then
		bash ${DESTINATION}/final.sh --add-motd
	else
		bash ${DESTINATION}/final.sh
	fi
	rm ${DESTINATION}/final..sh
}
